...
Aplicação desenvolvida para praticar o que foi aprendido no React.js junto com TypeScript, com o conceito de CRUD (Create, Read, Update, Delete). Essa será uma aplicação que irá se conectar a uma fake API, e exibir os pratos de comida criados e permitir a criação, remoção e atualização desses pratos, seguindo os requisitos propostos logo abaixo pelo desafio 10 do bootcamp GoStack da Rocketseat.
...
Para rodar a aplicação execute o comando `yarn start` ou `npm run start`
